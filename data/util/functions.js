...
module.exports = {
	...
	checkForUpdates: async function() {
		...
		await fetch('https://registry.npmjs.com/discord-turtle')
			.then((res) => res.json())
			.then((res) => {
				const versions = Object.keys(res.time);
				const vLatest = versions[versions.length - 1];
				if (package.dependencies.discord-turtle) {
					if (res['dist-tags'].latest !== package.dependencies.discord-turtle.slice(1)) {
						const msg = chalk(
							`new ${chalk.green('version')} of ${chalk.yellow(
								'discord-turtle',
							)} is available! ${chalk.red(
								package.dependencies.discord-turtle.slice(1),
							)} -> ${chalk.green(vLatest)}`,
						);
						const tip = chalk(
							`registry ${chalk.cyan('https://www.npmjs.com/package/discord-turtle')}`,
						);
						const install = chalk(
							`run ${chalk.green('npm i discord-turtle@latest')} to update`,
						);
						boxConsole([msg, tip, install]);
					}
				} else if (package.devDependencies.discord-turtle) {
					if (res['dist-tags'].latest !== package.devDependencies.discord-turtle.slice(1)) {
						const msg = chalk(
							`new ${chalk.green('version')} of ${chalk.yellow(
								'discord-turtle',
							)} is available! ${chalk.red(
								package.devDependencies.discord-turtle.slice(1),
							)} -> ${chalk.green(vLatest)}`,
						);
						const tip = chalk(
							`registry ${chalk.cyan('https://www.npmjs.com/package/discord-turtle')}`,
